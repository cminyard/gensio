...
func (e *EvHnd) wait(count uint, timeout *gensio.GensioTime) {
...
func (od *Open_Done) wait(count uint, timeout *gensio.GensioTime) {
	od.w.Wait(count, timeout)
}
func (od *Open_Done) Open_done(err int) {
	od.err = err
	od.w.Wake()
}

func main() {
	o := gensio.NewOsFuncs(0)

	acch := &AccEvHnd{}
	acc := gensio.GensioAccAlloc("tcp,localhost,0", o, acch)
	acc.Startup()
	port := acc.GetPort()
	fmt.Printf("Accepter port: %s\n", port)

	e := NewEvHnd(o)
	g := gensio.GensioAlloc("tcp,localhost," + port, o, e)
	e.g = g
	e.set_write_data([]byte("Test1"))

	od := NewOpen_Done(o)
	g.Open(od)
	od.wait(1, gensio.NewGensioTime(1, 0))
	if od.err != 0 {
	...
	data = append(data, "0"...)
	rv, newdata = g.Control(0, true, gensio.GENSIO_CONTROL_LADDR, data)
	fmt.Printf("laddr(%d): %s\n", rv, newdata)
	rv, newdata = g.Control(0, true, gensio.GENSIO_CONTROL_RADDR, data)
	...
	rv, newdata = g.ReadS(data, gensio.NewGensioTime(1, 0), false)
	fmt.Printf("Sync Read(%d): %s\n", rv, newdata)
	g.ClearSync()

	g.SetWriteCallbackEnable(true)
	t := gensio.NewGensioTime(check(), 0)
	e.wait(1, t)
	...
	e = NewEvHnd(o)
	g = gensio.GensioAlloc("serialdev,/dev/ttyEcho0,115200n81", o, e)

	runtime.GC()
	w := gensio.NewWaiter(o)
	w.Wait(uint(1), gensio.NewGensioTime(1, 0))
	runtime.GC()
	w.Wait(uint(1), gensio.NewGensioTime(1, 0))
	runtime.GC()
	w.Wait(uint(1), gensio.NewGensioTime(1, 0))
