...
type Gensio interface {
	...
	Control(depth int, get bool, option uint, data []byte) (int, []byte,
		uint64)
	Acontrol(depth int, get bool, option uint, data []byte,
		 cd GensioControlDone, timeout *Time) int
	AcontrolS(depth int, get bool, option uint, data []byte,
		  timeout *Time) (int, []byte, uint64)
...
type Event interface {
	...
	Modemstate(state uint)
	ModemstateMask(state uint)
	Linestate(state uint)
	LinestateMask(state uint)
	Signature(data []byte)
	FlowState(state bool)
	Sync()
	Baud(baud uint)
...
// Interface for handling a Control() operation completing.
type GensioControlDone interface {
	ControlDone(err int, data []byte)

	// Internal, do not use
	getGensioControlDoneBase() *GensioControlDoneBase
}

// This type must be the first entry in your event-handling callback
// for a Control() operation.
type GensioControlDoneBase struct {
	GensioControlDone
...
func (e *EventBase) Modemstate(state uint) { }
func (e *EventBase) ModemstateMask(state uint) { }
func (e *EventBase) Linestate(state uint) { }
func (e *EventBase) LinestateMask(state uint) { }
func (e *EventBase) Signature(data []byte) { }
func (e *EventBase) FlowState(state bool) { }
func (e *EventBase) Sync() { }
func (e *EventBase) Baud(baud uint) { }
func (e *EventBase) Datasize(size uint) { }
func (e *EventBase) Parity(par uint) { }
func (e *EventBase) Stopbits(bits uint) { }
func (e *EventBase) Flowcontrol(flow uint) { }
func (e *EventBase) Iflowcontrol(flow uint) { }
func (e *EventBase) Sbreak(sbreak uint) { }
func (e *EventBase) Dtr(dtr uint) { }
func (e *EventBase) Rts(rts uint) { }
func (e *EventBase) Flush(val uint) { }
...
type rawGensioControlDoneBase struct {
	RawGensio_Control_Done
	subd GensioControlDone
}

func (cd *GensioControlDoneBase) getGensioControlDoneBase() *GensioControlDoneBase {
	return cd
}

func (cd *rawGensioControlDoneBase) Control_done(err int, data []byte) {
	ndata := make([]byte, len(data))
	copy(ndata, data)
	cd.subd.ControlDone(err, ndata)
	cd.subd = nil // Break the circular reference
}

func setupGensioControlDone(cd GensioControlDone) RawGensio_Control_Done {
	cdb := cd.getGensioControlDoneBase()
	cdb.cd = &rawGensioControlDoneBase{}
	cdb.cd.subd = cd
	cdb.cd.RawGensio_Control_Done = NewDirectorGensio_Control_Done(cdb.cd)
	runtime.SetFinalizer(cdb, destroyer.destroy)
	return cdb.cd
}

func (cd *GensioControlDoneBase) destroy() {
	DeleteRawGensio_Control_Done(cd.cd)
	if Debug {
		fmt.Println("Control Done destroy")
	}
	atomic.AddUint32(&GCCount, 1)
}
...
func (g *gensioO) Acontrol(depth int, get bool, option uint,
			   data []byte, cd GensioControlDone,
			   timeout *Time) int {
	if cd == nil {
		return g.g.Acontrol_nodone(depth, get, option, data, timeout)
	} else {
		return g.g.Acontrol(depth, get, option, data,
			setupGensioControlDone(cd), timeout)
	}
}

func (g *gensioO) AcontrolS(depth int, get bool, option uint,
			    data []byte, timeout *Time) (int, []byte, uint64) {
	var len uint64
	var actlen uint64
	rv := g.g.Acontrol_s(depth, get, option, data, &len, timeout)
	actlen = len
	if actlen > uint64(cap(data)) {
		actlen = uint64(cap(data))
	}
