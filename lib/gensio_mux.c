		       ...
		       struct gensio **new_io)
{
    ...
    mux_lock(muxdata);
    chan = mux_new_channel(muxdata, cb, user_data, is_client);
    if (!chan) {
	err = GE_NOMEM;
	goto out;
    }
	       ...
	       const char *const *nauxdata)
{
	...
	if (muxdata->in_hdr) {
		...
		if (muxdata->state == MUX_WAITING_OPEN)
		    ...
		    chan = mux_new_channel(muxdata, NULL, NULL, false);
		...
		if (chan && chan->open_done) {
			...
			goto more_data;
		    if (chan->errcode) {
			if (chan->state == MUX_INST_IN_OPEN_CLOSE)
			    mux_channel_finish_close(chan);
			chan_deref(chan);
		...
		chan->errcode = mux_buf_to_u16(muxdata->hdr + 8);
