...
static int net_check_open(void *handler_data, struct gensio_iod *iod)
{
    ...
    tdata->last_err = gensio_os_check_socket_open(iod);
...
net_try_open(struct net_data *tdata, struct gensio_iod **iod)
{
 ...
 retry:
    err = gensio_os_connect(new_iod, tdata->ai);
	    ...
	    unsigned int option, char *data, gensiods *datalen)
{
	...
	if (get) {
	    if (iod) {
		rv = gensio_os_get_nodelay(iod, GENSIO_NET_PROTOCOL_TCP, &val);
		if (rv)
		    return rv;
	    } else {
		val = tdata->nodelay;
	    ...
	    *datalen = snprintf(data, *datalen, "%d", val);
	    ...
	    if (iod) {
		rv = gensio_os_set_nodelay(iod, GENSIO_NET_PROTOCOL_TCP, val);
		if (rv)
		    return rv;
	    }
	    tdata->nodelay = val;
	 ...
	 const char ***auxdata, void *cb_data)
{
	...
	*rcount = 1;
    ...
    return gensio_os_recv(iod, data, count, rcount, 0);
}
...
net_except_ready(void *handler_data, struct gensio_iod *iod)
{
    ...
    rv = gensio_os_recv(iod, &urgdata, 1, &rcount, GENSIO_MSG_OOB);
    if (rv || rcount == 0)
	return GE_NOTSUP;
	  ...
	  const char *const *auxdata)
{
    ...
    return gensio_os_send(iod, sg, sglen, rcount, flags);
}
...
netna_fd_cleared(struct gensio_iod *iod, void *cbdata)
{
    ...
    gensio_os_close(&nadata->acceptfds[i].iod);
		    ...
		    char *data, gensiods *datalen)
{
    ...
    rv = gensio_os_getsockname(nadata->acceptfds[i].iod, &addr);
    if (rv)
	return rv;
